...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	settingService := service.ProvideSettingService(settingRepository, groupRepository, userRepository, configConfig)
	...
	linuxDoCreditService := service.NewLinuxDoCreditService(client, paymentOrderRepository, userRepository, redeemCodeRepository, billingCacheService, apiKeyAuthCacheInvalidator, settingService)
	linuxDoCreditHandler := handler.NewLinuxDoCreditHandler(linuxDoCreditService)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, geminiTokenProvider, antigravityGatewayService, httpUpstream, configConfig)
	...
	subscriptionRequestProxyRouter := service.NewSubscriptionRequestProxyRouter(proxyRepository)
	usageRecordWorkerPool := service.NewUsageRecordWorkerPool(configConfig)
	userMsgQueueCache := repository.NewUserMsgQueueCache(redisClient)
	userMessageQueueService := service.ProvideUserMessageQueueService(userMsgQueueCache, rpmCache, configConfig)
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, subscriptionRequestProxyRouter, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, userMessageQueueService, configConfig, settingService)
	openAIGatewayHandler := handler.NewOpenAIGatewayHandler(openAIGatewayService, subscriptionRequestProxyRouter, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, configConfig)
	...
	soraGatewayHandler := handler.NewSoraGatewayHandler(gatewayService, soraGatewayService, subscriptionRequestProxyRouter, concurrencyService, billingCacheService, usageRecordWorkerPool, configConfig)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, linuxDoCreditHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, soraGatewayHandler, soraClientHandler, handlerSettingHandler, totpHandler, idempotencyCoordinator, idempotencyCleanupService)
