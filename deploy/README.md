# b022hub Deployment Files

This directory contains files for deploying b022hub on Linux servers.
...
curl -sSL https://raw.githubusercontent.com/B022MC/b022hub/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
Set these variables in `deploy/.env` before starting Docker Compose:
...
- Do not use `localhost`, `127.0.0.1`, or a local filesystem path unless the orchestrator is on the same host.
- After deployment, you can also update the endpoint online in `Admin -> Relogin Queue -> Relogin Service Endpoint`. Those runtime settings are stored in the database and take effect without restarting `sub2api`.
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf b022hub-complete.tar.gz deployment/

# Transfer to new server
scp b022hub-complete.tar.gz user@new-server:/path/to/destination/
...
b022hub supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/B022MC/b022hub/releases)
...
b022hub supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).
