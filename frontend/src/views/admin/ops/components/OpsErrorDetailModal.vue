<template>
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="ops-card-muted rounded-xl p-6">
        ...
        <pre class="ops-card-subtle mt-4 max-h-[520px] overflow-auto rounded-xl p-4 text-xs text-gray-800 dark:text-gray-100"><code>{{ prettyJSON(primaryResponseBody || '') }}</code></pre>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="ops-card-muted rounded-xl p-6">
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-gray-900 dark:text-white">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ops-pill-muted ml-2 rounded-md px-2 py-0.5 font-mono text-[10px] font-bold">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="ops-link inline-flex items-center gap-1.5 rounded-md px-1.5 py-1 text-[10px] font-bold disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <pre
              ...
              class="ops-card-muted mt-3 max-h-[240px] overflow-auto rounded-xl p-3 text-xs text-gray-800 dark:text-gray-100"
