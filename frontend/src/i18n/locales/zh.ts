export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportWarning: '导入将创建新账号与代理；如选择分组，导入的账号会自动绑定到这些分组。请确认已有数据不会冲突。',
      dataImportGroupHint: '所选分组会应用到本次导入的所有账号；留空则保持当前行为，不自动绑定分组。',
      dataImportFile: '数据文件',
      dataImportFileHint: 'JSON (.json)，支持多选',
      ...
      dataImportResultSummary: '文件成功 {file_successful}/{file_total}，失败 {file_failed}；代理创建 {proxy_created}，复用 {proxy_reused}，失败 {proxy_failed}；账号创建 {account_created}，失败 {account_failed}',
      dataImportErrors: '失败详情',
      dataImportSuccess: '导入完成：文件成功 {file_successful}/{file_total}，账号 {account_created}，失败 {account_failed}',
      dataImportCompletedWithErrors: '导入完成但有错误：文件失败 {file_failed}，账号失败 {account_failed}，代理失败 {proxy_failed}',
    ...
    proxies: {
      ...
      dataImportHint: '支持导入导出的 JSON 文件，或直接导入 Clash 订阅链接。',
      dataImportWarning: '导入将创建或复用代理，保留状态并在完成后自动触发延迟检测。不支持的订阅节点类型会跳过。',
      dataImportSource: '导入来源',
      dataImportModeFile: 'JSON 文件',
      dataImportModeSubscription: '订阅链接',
      dataImportFile: '数据文件',
      dataImportSubscriptionUrl: '订阅链接',
      dataImportSubscriptionPlaceholder: 'https://example.com/clash.yaml',
      dataImportSubscriptionHint: '当前只导入 http/https/socks5/socks5h 节点；ss、vmess、trojan 等类型会跳过。',
      dataImportButton: '开始导入',
      dataImporting: '导入中...',
      dataImportSelectFile: '请选择数据文件',
      dataImportSelectSubscriptionUrl: '请输入订阅链接',
      dataImportParseFailed: '数据解析失败',
      dataImportFailed: '数据导入失败',
      dataImportResult: '导入结果',
